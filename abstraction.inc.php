	...
	class Min_MySQLi extends MySQLi {
		...
		function connect($server, $username, $password) {
			return @$this->real_connect(
				...
				(strlen("$server$username$password") ? $\1REDACTED_SECRET"mysqli.default_pw"))
			);
		}
	...
	class Min_MySQL {
		...
		function connect($server, $username, $password) {
			$this->_link = @mysql_pconnect(
				...
				(strlen("$server$username$password") ? $\1REDACTED_SECRET"mysql.default_password")),
